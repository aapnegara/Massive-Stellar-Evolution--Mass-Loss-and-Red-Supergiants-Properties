...
for i in 0 1; do
	...
	change 'log_directory' "'LOGS_${M_ZAMS_array[i]}M0'" 'inlist_star'
	change 'photo_directory' "'photos_${M_ZAMS_array[i]}M0'" 'inlist_star'
    
    if [ -d LOGS_${M_ZAMS_array[i]}M0 ]; then
        cd LOGS_${M_ZAMS_array[i]}M0
	    gvfs-trash *
	    cd ../
    fi    
    if [ -d photos_${M_ZAMS_array[i]}M0 ]; then
        cd photos_${M_ZAMS_array[i]}M0
	    gvfs-trash *
	    cd ../
    fi
	./mk
