...
! ***********************************************************************
	...
	subroutine  dJ_M_wind(id, Lsurf, Msurf, Rsurf, Tsurf, w, ierr)                                                                  
	     use crlibm_lib
	     type (star_info), pointer :: s
	...
	if (T1 < 5012) then
	       ...
	       w = w * etaCOOL
                ...
                print *, 'using: de_Jager, eta=', etaHOT
	...
	subroutine eval_Meynet_wind(w)
        ! From Meynet et al. 2015
         real(dp), intent(out) :: w
            real(dp) :: log10w
            include 'formats'
            log10w = log10_cr(L1/Lsun)*1.84 - 14.3
            w = exp10_cr(log10w)
