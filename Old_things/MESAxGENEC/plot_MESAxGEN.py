...
gen_list = [pd.read_csv("M0%iZ14V0.dat" % mass,sep=" ") for mass in mass_list]
...
[plt.plot(gen_list[i]["lg(Teff)"][:-240],gen_list[i]["lg(L)"][:-240],color = colorlist[i],linestyle="-.", label = '$GENEC: %iM_{\odot}$' % mass_list[i]) for i in range(len(gen_list))]
